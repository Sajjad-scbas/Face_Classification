...
class SwinTransformer(nn.Module):
    def __init__(self, parameters):
        ...
        self.patching_x = PatchEmbedding(parameters)
        ...
        self.encode1 = StageModule(parameters=parameters)


        self.down_sampling.append(Down_Sampling(dim=parameters['hidden_emb']))
    
        self.up_sampling.append(Up_Sampling(dim= parameters['hidden_emb']))
        
        
        self.output = PactchRecovery(parameters)
        
        parameters['hidden_emb'] = 2*parameters['hidden_emb']
        parameters['head_dim'] = 2*parameters['head_dim']
        
        
        self.encode2 = StageModule(parameters=parameters)
        ...
        self.encode3 = StageModule(parameters=parameters)
    ...
    def forward(self, x):
        ...
        x = self.patching_x(x) # x = (P, H, W, C=96)
